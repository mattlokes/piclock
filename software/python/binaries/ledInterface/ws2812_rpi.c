...
void show_argb_matrix16x16( aColor_t* frame) {
	...
	for (i = 0; i < 16; i++) {
		...
		for (j = 0; j < 16; j++) {
		   //printf("RGB: %d, %d, %d\n", LEDBuffer[i].r, LEDBuffer[i].g, LEDBuffer[i].b);
		   pixelPtr->r *= brightness;
		   pixelPtr->g *= brightness;
		   pixelPtr->b *= brightness;
		   colorBits = ((unsigned int)pixelPtr->r << 8) | ((unsigned int)pixelPtr->g << 16) | pixelPtr->b;
		   //printBinary(colorBits, 32);
		   //printf(" (binary, GRB order)\n");

		   // Iterate through color bits to get wire bits
		   for(k=23; k>=0; k--) {
			   colorBit = (colorBits & (1 << k)) ? 1 : 0;
			   switch(colorBit) {
				   case 1:
					   //wireBits = 0b110;	// High, High, Low
					   setPWMBit(wireBit++, 1);
